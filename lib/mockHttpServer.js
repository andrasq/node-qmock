...
MockServer.prototype.when = function when( condition ) {
    ...
    if (typeof condition === 'string') {
        ...
        tester = function(req, res) {
            ...
            var pathname = url.slice(url.indexOf('/', url.indexOf('://') + 3));
            var method = req.method + ':';
            return pathname === condition || url === condition ||
                method + pathname === condition || method + url === condition;
        };
    }
 ...
 */
MockServer.prototype.makeRequest = function makeRequest( url, body, headers ) {
    this.actions.push(function _makeRequest(req, res, next) {
        url = url || req.url;

        var uri = (typeof url === 'string') ? Url.parse(url) : util._extend({}, url);

        if (headers) uri.headers = headers;

        var request = (uri.protocol === 'https:' || uri.protocol === undefined && uri.port == 443) ? mockHttp._methods.sysHttpsRequest : mockHttp._methods.sysHttpRequest;
console.log("AR: ireq uri", uri, request);
        var realReq = request(uri, function(realRes) {
        ...
        // makeRequest() whitelists the url and makes an actual web request
        ...
        var mockWrite = req.write;
        var mockEnd = req.end;

        if (body) {
            // with a body send it as the request body
            realReq.write(body);
            req.write(body);
        }
        else {
            // without a body replay the request to the actual request
            for (var i=0; i<req._mockWrites.length; i++) {
                if (req._mockWrites[i]) realReq.write(req._mockWrites[i][0], req._mockWrites[i][1]);
            }
        }

        // relay any other writes that arrive
        req.write = function(chunk, encoding) {
            mockWrite.call(req, chunk, encoding)
            realReq.write(chunk, encoding);
        };

        // relay the request end
        req.end = function() {
            mockEnd.call(req)
            realReq.end();
        };

        // if the mock request has been ended, end the real request too
        if (req._mockWrites.indexOf(null) >= 0) realReq.end();
    })
