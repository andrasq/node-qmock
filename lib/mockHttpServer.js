...
MockServer.prototype.when = function when( condition ) {
    ...
    if (typeof condition === 'string') {
        ...
        tester = function(req, res) {
            ...
            // To match on the query path not also the query string params, uncomment:
            // if ((mark = pathname.indexOf('#')) >= 0) pathname = pathname.slice(0, mark);
            // if ((mark = pathname.indexOf('?')) >= 0) pathname = pathname.slice(0, mark);
...
MockServer.prototype.makeRequest = function makeRequest( url, body, headers ) {
    this.actions.push(function _makeRequest(req, res, next) {
        ...
        var uri = (typeof url === 'string') ? Url.parse(url) : url;
        uri.method = uri.method || req.method || 'GET';
