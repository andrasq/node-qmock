...
function mockRequest( httpHandler, defaultOptions ) {
    return function _mockRequestCall( uri, callback ) {
        ...
        if (uri.auth) req.setHeader('authorization', 'Basic ' + new Buffer(uri.auth).toString('base64'));
...
// FIXME: req.url should be just the path, not the full url.
// Notably, req.url is not enough for a call replay.
// The full url can be build from req._options
