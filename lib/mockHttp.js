...
if (!global.___qmock_http_request) Object.defineProperty(global, '___qmock_http_request', { value: http.request, enumerable: false });
if (!global.___qmock_https_request) Object.defineProperty(global, '___qmock_https_request', { value: https.request, enumerable: false });
...
function mockRequest( httpHandler, defaultOptions ) {
    return function _mockRequestCall( uri, callback ) {
        ...
        // start the response on first write (or end), or after 30 ms, whichever occurs first
        // actual response emulation is up to the httpHandler fake server, eg mockHttpServer
        var responseStarted = false;
        function startResponseOnWrite() {
            if (!responseStarted++) setImmediate(function() {
        ...
        setTimeout(startResponseOnWrite, 30);
