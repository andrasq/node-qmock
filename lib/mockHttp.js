...
function mockRequest( httpHandler, defaultOptions ) {
    return function _mockRequestCall( uri, callback ) {
        ...
        // suppress req.setTimeout() errors; simulate a timeout with .delay and .throw
        req.sock = new Stream();
        req.sock.setTimeout = function() {};
        ...
        req.once('mockResponse', function(handlerRes) {
            // TODO: fix res._headers to contain response headers
            // TODO: why does res.headers contain { host: 'localhost:1337' } ??
        ...
        setImmediate(function(){
            req.url = options.href ? options.href : Url.format(options);
            httpHandler(req, res);
        })
