...
function recordMethod( stub, interceptedMethod, interceptedThisObject, args ) {
    ...
    if (interceptedMethod === 'stub') {
        ...
        var ret = stub._retvals[0] || new Retval().once(true);
        if (ret._once) stub._retvals.shift();
        if (ret.yields) {
            // note: nodejs v8 does not guarantee that the clock will have advanced by 2 ms
            if (ret._async) setTimeout(function(){ cb.apply(null, ret.yields) }, 2);
...
function Retval( err, ret, yields ) {
...
Retval.prototype.once = function _once( yesno ) { this._once = yesno; return this; }
Retval.prototype.async = function _async( yesno ) { this._async = yesno; return this; }
Retval.prototype = Retval.prototype;
...
function QmockStubContext( object, methodName, overrideMethod, thisObject ) {
    ...
    this._defaultRetval = this._retvals[0];     // the retval set by yields/returns/throws
...
QmockStubContext.prototype.yields = function _yields( /* VARARGS */ ) {
    ...
    this._defaultRetval.yields = args;
    return this;
}
QmockStubContext.prototype.yieldsAsync = function _yieldsAsync( /* VARARGS */ ) {
    ...
    this._overrideMethod = 'stub';
    this._defaultRetval.yields = args;
    this._defaultRetval.async(true);
    return this;
}
QmockStubContext.prototype.returns = function _returns( val ) {
    // TODO: returns(val), returnsArg(n), returnsThis()
    this._overrideMethod = 'stub';
    this._defaultRetval.ret = val;
    return this;
}
QmockStubContext.prototype.throws = function _throws( err ) {
    // TODO: (), ("TypeName"), (obj)
    this._overrideMethod = 'stub';
    this._defaultRetval.isError = true;
    this._defaultRetval.err = err;
    return this;
}
QmockStubContext.prototype.resolves = function( value ) {
    /* istanbul ignore next */
    return this.returns(global.Promise ? global.Promise.resolve(value) : {
        then: function(cb) { cb(value) },
        catch: function(cb) {},
    })
}
QmockStubContext.prototype.rejects = function( err ) {
    /* istanbul ignore next */
    return this.returns(global.Promise ? global.Promise.reject(err) : {
        then: function(cb) { },
        catch: function(cb) { cb(err) },
    });
}
QmockStubContext.prototype.onCall = function _onCall( index ) {
    // onCall() controls which retval will be configured by returns/yields/throws.
    // The default is the last (permanent) retval.
    if (index >= 0) {
        // _mockOnce creates retvals, but expects 1-based indexes
        this._mockOnce(index + 1);
        this._defaultRetval = this._retvals[index];
    }
    else {
        // reset the default back to the last (permanent) retval
        this._defaultRetval = this._retvals[this._retvals.length - 1];
    }
    return this;
}
QmockStubContext.prototype._mockOnce = function __mockOnce( index, err, ret, yields, async ) {
    if (yields != null && !Array.isArray(yields)) throw new Error("_mockOnce: yields expects an array");
    this._overrideMethod = 'stub';
    var retval = this._retvals.pop();
    if (index > 0) {
        // with an index make the default the retval at that call slot (for onCall)
        // Grow the retvals list if the call slot is not already present
        this._retvals[index - 1] = this._retvals[index - 1] || new Retval().once(true);
    } else {
        // without an index append a new use-once retval, but leave the default
        this._retvals.push(new Retval(err, ret, yields).once(true).async(async));
    }
...
QmockStubContext.prototype = QmockStubContext.prototype;
