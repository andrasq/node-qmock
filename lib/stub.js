...
function spy( objectOrFunction, methodName, overrideMethod, options ) {
    ...
    if (!options) options = { saveLimit: 10, stubWithSelf: true };
    ...
    if (typeof objectOrFunction === 'object' && typeof methodName === 'string') {
        ...
        return stub(object, methodName, overrideMethod, { saveLimit: options.saveLimit || 10 });
    }
    else {
        ...
        var spyStub = stub({}, 'anon', func, { saveLimit: options.saveLimit || 10 });
...
function _stubOnce( func, object, methodName, overrideMethod, options ) {
    ...
    info._object[stub._methodName] = interceptCall(stub, methodName, object, handler);
    return info;

    function handler( stub, method, self, args ) {
        try {
            var ret = recordMethod(info, stub._overrideMethod, object, args);
            stub.restore();
            return ret;
        }
        catch (err) {
