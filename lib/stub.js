...
// FIXME: if objectOrFunction is a constructor function, should mock as a class, not as a function!!
        ...
        // returns a spy function with a `stub` property and a `restore()` method
        // calling restore() returns the original spied-on function
        spyFunc.stub = spyStub;                 // spied stats
        spyFunc.restore = function(){ return func };
        return spyFunc;
    }
}
...
function stub( object, methodName, overrideMethod, options ) {
    ...
    if (!object) return (typeof methodName === 'function')
        ? spy(methodName, null, null, { saveLimit: 0 })
        : spy(null, null, null, { saveLimit: 0 });

    if (typeof overrideMethod !== 'function' && options == undefined) { options = overrideMethod; overrideMethod = undefined; };
    if (!options || typeof options !== 'object') options = { saveLimit: 0, stubWithSelf: false };
