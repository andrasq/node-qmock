...
function spy( objectOrFunction, methodName, overrideMethod ) {
    if (objectOrFunction && methodName) {
        // spy on calls to the method, return a stub that tracks the calls
        var object = objectOrFunction;
        return stub(object, methodName, overrideMethod, { saveLimit: 10 });
    }
    else {
        ...
        var spyStub = stub({}, 'anon', func, { saveLimit: 10 });
        ...
        spyFunc.restore = function(){ throw new Error("can only restore a method onto an object, not a bare function") };
        return spyFunc;
    }
}
