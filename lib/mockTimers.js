...
function overrideTimers( mockTimers ) {
    // TODO: omit for node v0.8?
    global.setImmediate = mockTimers.setImmediate;
    global.clearImmediate = mockTimers.clearImmediate;
...
function MockTimers( ) {
    ...
    self.setInterval = function mockSetInterval( func, ms ) {
        var av = new Array();
        for (var i=2; i<arguments.length; i++) av.push(arguments[i]);
...
MockTimers.prototype._processTimeouts = function _processTimeouts( ) {
    ...
    if (this.timeouts[timestamp]) {
        ...
        for (var i=0; i<timeouts.length; i++) {
            ...
            if (err) {
                this.timeouts[timestamp] = timeouts.slice([i+1]).concat(this.timeouts[timestamp]);
                var self = this;
