...
/**
 * create a mock object for the mocked class or object.
 * The mock is a fully functional shallow clone of the actual.
 */
function getMock( constructor, methods, constructorArgs ) {
    ...
    if (typeof constructor === 'function') {
        ...
        // TODO: how to guard against caller munging (editing) mocked object?
        // TODO: how to correctly handle overridden inherited method?
        //       ie self.method and self.super_.method?
        // note: cannot npm clone() the actual, it breaks on sockets
        // mock = clone(constructor);
        actual = constructor;
        mock = {};
        // quick shallow clone:
        mock.super_ = actual.super_;
        mock.__proto__ = actual.__proto__;
        for (i in actual) {
            if (i in QMock.prototype) throw new Error("cannot inherit existing QMock method " + i);
            if (actual.hasOwnProperty(i)) mock[i] = actual[i];
        }
    }

    if (!Array.isArray(methods)) {
        // if no methods named to be mocked, stub out all of them!
        // TODO: confirm that all functions have type 'function'
        for (i in mock) if (typeof mock[i] === 'function') mock[i] = function(){ };
    }
    else {
        // override the named mocked methods with no-ops
        for (i in methods) mock[methods[i]] = function(){ };
    }
...
function extendWithMocks( tester, doneMethod ) {
    ...
    tester.getMock = function(constructor, method, constructorArgs) {
        var mock = Mock.getMock(constructor, method, constructorArgs);
...
Expected.prototype.onConsecutiveCalls = function( willReturnValues ) {
    ...
    var i;
// FIXME: keep stack of QMockReturnedValue, which can specify eg point-of-return exceptions
...
Expected.prototype.execute = function( methodName, mockid, arglist ) {
    ...
    if (this.willReturn !== undefined) {
        // if return value provided, fake the call
        var returnValue;
        if (this.willReturn[0] instanceof QMockReturnValue) {
            returnValue = this.willReturn[0].get();
            if (this.willReturn[0].isEmpty()) this.willReturn.shift();
        }
        else {
            returnValue = this.willReturn.shift();
        }
...
function QMockExpectsValue( value ) {
    this.type = (typeof value === 'number') ? 'number' : 'action';
    this.value = value;
}
...
QMock.onConsecutiveCalls = function onConsecutiveCalls( valueList ) {
    // FIXME: this needs to push multiple values onto the willReturn stack...
    if (!Array.isArray(valueList)) throw new Error("onConsecutiveCalls takes an array of arrays");
    return new QMockReturnValue('multi', valueList);
};
QMock.throwExecption = function throwException( err ) {
    return function(){ throw err; };
}
QMock.throwException = function throwError( err ) {
...
function _cloneArray( array, offset ) {
    var i, list = [];
    offset = offset || 0;
    for (i=offset; i<array.length; i++) list.push(arguments[i]);
    return list;
}
