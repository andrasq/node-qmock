...
function getMock( mocked, methodsToStub, constructorArgs ) {
    ...
    if (typeof mocked === 'function') {
        ...
        setPrototype(mock, constructor.prototype);
    }
    else {
        ...
        if (actual.__proto__ !== undefined) setPrototype(mock, actual.__proto__);
    }
...
function setPrototype( object, proto ) {
    // TODO: instead of assigning, copy the prototype object
    // TODO: however, the constructor is not enumerable, breaks instanceof
    object.__proto__ = proto;
    // note: making __proto__ non-enumerable makes it read-only and prevents assignment
    //Object.defineProperty(object, '__proto__', { enumerable: false });
    return object;
}
...
function extendWithMocks( tester, doneMethod ) {
    ...
    tester.__qmockList = [];
    tester.getMock = function getMock(constructor, method, constructorArgs) {
        var mock = QMock.getMock(constructor, method, constructorArgs);
        tester.__qmockList.push(mock);
        return mock;
    };
    tester.getMockSkipConstructor = function getMockSkipConstructor(constructor, method) {
        var mock = QMock.getMock(constructor, method);
        tester.__qmockList.push(mock);
        return mock;
    };
...
function QMock( actual ) {
    ...
    if (!this.__qmock__._hasCheckMethod) this.check = QMock.prototype.check;
}
